...
public class OptionsService {
    ...
    public void isOptionFromQuestion(Question question, Options option){
        ...
        if(!question.getId().equals(option.getQuestion().getId())){
    ...
    public OptionResponseDto createOption(Long userId, Long surveyId, Long questionId, OptionCreateRequestDto requestDto){
        ...
        isCurrentUserSurveyee(userId, survey, "참여자 권한으로는 선택지를 생성할 수 없습니다.");
    ...
    public List<OptionResponseDto> getOptions(Long userId, Long surveyId, Long questionId){
        ...
        if(!status.equals(SurveyStatus.IN_PROGRESS)){
            if(user.getUserRole().equals(UserRoleEnum.SURVEYEE)){
                throw new CustomException(ErrorCode.SURVEYEE_NOT_ALLOWED, "진행 중 상태가 아닌 설문의 선택지는 설문 참여자 권한으로 조회할 수 없습니다.");
            }
    ...
    public OptionResponseDto updateOption(Long userId, Long surveyId, Long questionId, Long optionId, OptionUpdateRequestDto requestDto){
        ...
        isCurrentUserSurveyee(userId, survey, "참여자 권한으로는 선택지를 수정할 수 없습니다.");
        ...
        if(requestDto.getNumber() != null){
            option.setNumber(requestDto.getNumber());
        }
        if(requestDto.getContent() != null){
            option.setContent(requestDto.getContent());
        }
    ...
    public void deleteOption(Long userId, Long surveyId, Long questionId, Long optionId){
        ...
        isCurrentUserSurveyee(userId, survey, "참여자 권한으로는 선택지를 삭제할 수 없습니다.");
