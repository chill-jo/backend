...
public class QuestionService {
    ...
    public void isCurrentUserSurveyee(User user, String message){
    ...
    public QuestionResponseDto createQuestion(User user, Long surveyId, QuestionCreateRequestDto requestDto){

        Survey survey = surveyRepository.findById(surveyId)
                .orElseThrow(() -> new CustomException(ErrorCode.SURVEY_NOT_FOUND));

        isCurrentUserSurveyee(user, "참여자 권한으로는 질문을 생성할 수 없습니다.");
        currentUserMatchesSurveyCreator(user, survey, "설문 출제자와 관리자만 질문을 생성할 수 있습니다.");
    ...
    public QuestionResponseDto getQuestion(User user, Long surveyId, Long questionId){
        Survey survey = surveyRepository.findById(surveyId)
                .orElseThrow(() -> new CustomException(ErrorCode.SURVEY_NOT_FOUND));
        ...
        if(!survey.isSurveyStatusInProgress()){
            isCurrentUserSurveyee(user, "진행 중 상태가 아닌 설문의 질문은 설문 참여자 권한으로 조회할 수 없습니다.");
            currentUserMatchesSurveyCreator(user, survey, "설문 출제자와 관리자만 진행 중이 아닌 설문의 질문을 조회할 수 있습니다.");
        }
    ...
    public PageQuestionResponseDto<QuestionResponseDto> getQuestions(int page, int size, User user, Long surveyId){
        Survey survey = surveyRepository.findById(surveyId)
                .orElseThrow(() -> new CustomException(ErrorCode.SURVEY_NOT_FOUND));

        if(!survey.isSurveyStatusInProgress()){
            isCurrentUserSurveyee(user, "진행 중 상태가 아닌 설문의 질문은 설문 참여자 권한으로 조회할 수 없습니다.");
            currentUserMatchesSurveyCreator(user, survey, "설문 출제자와 관리자만 진행 중이 아닌 설문의 질문 목록을 조회할 수 있습니다.");
        }
    ...
    public QuestionResponseDto updateQuestion(User user, Long surveyId, Long questionId, QuestionUpdateRequestDto requestDto){
        ...
        isCurrentUserSurveyee(user, "참여자 권한으로는 질문을 수정할 수 없습니다.");
        currentUserMatchesSurveyCreator(user, survey, "설문 출제자와 관리자만 질문을 수정할 수 있습니다.");
    ...
    @Transactional
    public void deleteQuestion(User user, Long surveyId, Long questionId){
        ...
        isCurrentUserSurveyee(user, "참여자 권한으로는 질문을 삭제할 수 없습니다.");
        currentUserMatchesSurveyCreator(user, survey, "설문 출제자와 관리자만 질문을 삭제할 수 있습니다.");
